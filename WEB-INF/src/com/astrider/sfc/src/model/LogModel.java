...
public class LogModel extends BaseModel {
	public boolean cookComplete(HttpServletRequest request) {
		...
		Validator<CookCompleteVo> validator = new Validator<CookCompleteVo>(
				cookComplete);
		if (!validator.valid()) {
		...
		UserStatsVo userStats = userStatsDao.selectByUserId(loginUser
				.getUserId());
		userStats.setTotalCooked(userStats.getTotalCooked() + 1);
		if (1 == cookLogDao.countCookedAtToday(loginUser.getUserId())) {
			userStats
					.setConsecutivelyCooked(userStats.getConsecutivelyCooked() + 1);
			if (userStats.getMaxConsecutivelyCooked() < userStats
					.getConsecutivelyCooked()) {
				userStats.setMaxConsecutivelyCooked(userStats
						.getConsecutivelyCooked());
			}
		}
		...
		boolean succeed = mealNutAmountsDao.insertFromRecipeNutAmounts(
				loginUser.getUserId(), cookLog.getRecipeId());
		if (!succeed) {
			flashMessage.addMessage("食事別栄養素の挿入に失敗しました");
			flashMessage.setMessageType(Type.WARNING);
			return false;
		}
		MealLogVo mealNutAmounts = mealNutAmountsDao
				.selectNewestByUserId(loginUser.getUserId());
		mealNutAmountsDao.close();

		// 週別栄養素ログ更新
		WeeklyLogUtils.addMealToCurrentLog(loginUser.getUserId(),
				mealNutAmounts);
		WeeklyLogUtils.updateCurrentTotalBalance(loginUser.getUserId());

		// totalBalance更新
		WeeklyLogVo weekLog = WeeklyLogUtils.getCurrentLog(loginUser
				.getUserId());
