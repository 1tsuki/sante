...
/* Last update 2012.3.2 by taka */

var s_account="rcrttraining19prd"
...
s.linkInternalFilters="javascript:,172.20.115.89"
...
s.siteID="sante"					// leftmost value in pagename
...
/*
 * patch s.mr
 * facebook ie dedup patach for H.22.1
 */
s.mr = function (sess, q, rs, id, ta, u) {
var s = this, dc = s.dc, t1 = s.trackingServer, t2 = s.trackingServerSecure, tb = s.trackingServerBase; 
var p = ".sc", ns = s.visitorNamespace, un = s.cls(u ? u : ns ? ns : s.fun), r = new Object, l, imn = "s_i_" + un, im, b, e;
if (!rs) {if (t1) {if (t2 && s.ssl) {t1 = t2;}} else {if (!tb) {tb = "2o7.net";}
if (dc) {dc = ("" + dc).toLowerCase();} else {dc = "d1";}
if (tb == "2o7.net") {if (dc == "d1") {dc = "112";} else if (dc == "d2") {
dc = "122";}p = "";}t1 = un + "." + dc + "." + p + tb;}
if (q.indexOf("fb_xd_fragment") > -1) {s.fbxd=1;}
if (!s.fbxd){
rs = "http" + (s.ssl ? "s" : "") + "://" + t1 + "/b/ss/" + s.un + "/" 
+ (s.mobile ? "5.1" : "1") + "/H.22.1/" + sess + "?AQB=1&ndh=1" + (q ? q : "") 
+ "&AQE=1";
}else{rs=""; }
if (s.isie && !s.ismac) {rs = s.fl(rs, 2047);}if (id) {s.br(id, rs);return;}}
if (s.d.images && s.apv >= 3 && (!s.isopera || s.apv >= 7) && (s.ns6 < 0 || s.apv >= 6.1)) {
if (!s.rc) {s.rc = new Object;}if (!s.rc[un]) {s.rc[un] = 1;if (!s.rl) {s.rl = new Object;}
s.rl[un] = new Array;setTimeout("if(window.s_c_il)window.s_c_il[" + s._in + "].mrq(\"" + un + "\")", 750);
} else {l = s.rl[un];if (l) {r.t = ta;r.u = un;r.r = rs;l[l.length] = r;return "";}
imn += "_" + s.rc[un];s.rc[un]++;}im = s.wd[imn];if (!im) {im = s.wd[imn] = new Image;}im.s_l = 0;
im.onload = new Function("e", "this.s_l=1;var wd=window,s;if(wd.s_c_il){s=wd.s_c_il[" + s._in + 
"];s.mrq(\"" + un + "\");s.nrs--;if(!s.nrs)s.m_m(\"rr\")}");
if (!s.nrs) {s.nrs = 1;s.m_m("rs");} else {s.nrs++;}
im.src = rs;if ((!ta || ta == "_self" || ta == "_top" || s.wd.name && ta == s.wd.name) && rs.indexOf("&pe=") >= 0) {
b = e = new Date;while (!im.s_l && e.getTime() - b.getTime() < 500) {
e = new Date;}}return "";}
